...
The background can be organized into particular processes:
...
DThe fit strategies proposed are both clearly effective, but they both have their shortcomings:
...
Satisfied that the shape is good, we move swiftly on to view some of the example distributions given by @eq-sflow using this flow and one of the results from the many training configuration tried in @sec-sh-flows. We find these distributions in @fig-sflow-shapes, where we plot the value of $s_{\text{flow}}$ for
