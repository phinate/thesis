...
This simulation was done using the event generator PYTHIA, where the $X\rightarrow SH \rightarrow b\bar{b}\gamma\gamma$ process was simulated to leading order, i.e. only using the least complicated Feynman diagrams that can make this process happen.
...
I performed the training procedure as described for 5 flows with different initializations, and use their ensemble as the predictive model, with the standard deviations of these predictions (for likelihood values or histogram yields) acting as an uncertainty.
...
## A new flow-based observable

In addition to the work above, I had one more idea for this analysis. The fit strategies proposed are both clearly effective, but they both have their shortcomings:

- The 2-D fit is *simple* and *powerful*, but suffers from the *high number of bins* needed to compute the likelihood.
- The pNN strategy is much more *efficient*, as a 1-D distribution is far more simple to compute from an inference perspective. However, we need to *assign signal masses to background*, which isn't a desirable thing to encode.
