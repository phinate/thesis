...
One commonly used notion is the approximation of a Poisson distribution with a Normal distribution for large $\lambda$. We can see this demonstrated in @fig-poisson, where from $\lambda \approx 10$, we start to see the poisson count
...
def plot(dct):
    ...
    ax.scatter(grid,pdf, label="Poisson($\lambda$)")
    ax.plot(grid, pdf2, label="Normal($\lambda,\sqrt{\lambda}$)")
