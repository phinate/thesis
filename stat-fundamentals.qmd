...
$$ p(x_0) = \int_{x_0}^{+\infty}p(t(x) | H_0) dx ~.$$

We can then rephrase a hypothesis test as rejecting $H_0$ when $p(x_0) < \alpha$, and accepting $H_0$ otherwise. A low $p$-value is then a sign of low compatibility between the observed data and $H_0$.
...
It was probably hard to see the number of times I defined "extreme" in this very specific way that seems to apply to both intervals and tests. That's because they're one-to-one in many ways!

Say we make a $\mathrm{C.L.} = 95%$ confidence interval for $\mu$, given an ordering principle, a pdf $p(x | \mu)$, and some observed sata $x_0$.
