...
This idea was first used in @made, which used it to output the density directly in one forward pass by having the outputs correspond to the individual conditional distributions. However, to use this network in a flow, which transforms one distribution to another, we can instead use the outputs of the neural network to parametrize the transform $T$.

...


As a bonus, there's a simple way to condition this transform on side information $\mathbf{y}$: we can just add it as an extra input to the network, and have it mix with every input, since each conditional distribution would also include $\mathbf{y}$. We then have a flow that can perform *conditional density estimation, i.e. estimate $p(x_1, \dots, x_N| \mathbf{y}) =\prod_{i=1}^Np(x_i|\mathbf{x}_{j<i}, \mathbf{y})$.
