...
Now, notice that we wrote this holding for $g(\theta, \varphi) = 0$, which we don't have quite yet. But we can define this for our update state $f$ by simply letting $g(\theta, \varphi) = f(\theta, \varphi) - \theta$. Now, when we arrive at our solution $\theta_0$ as the fixed point of $f$ for some $\varphi_0$, we'll get $g(\theta_0, \varphi_0) = \theta_0 - \theta_0 = 0$, and we get all those nice properties! We just need to explicitly calculate the gradient of $\theta^*$ now, which we can do by differentiating both sides of $g(\theta^*(\varphi), \varphi) =0$:
...
$$
    \frac{\partial\hat{\theta}}{\partial\varphi} = -\left[\frac{\partial g}{\partial \theta}\right]^{-1}  \frac{\partial g}{\partial \phi} = \left[I - \frac{\partial f}{\partial \theta} \right]^{-1} \frac{\partial f}{\partial \varphi}
$$
