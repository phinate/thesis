---
title: "To the Standard Model of Particle Physics, and Beyond"
format:
  html:
    code-fold: true
...
An innumerable number of particle physics results in the last 10 years or so have shown no signs of deviating from the predictions made from the **Standard Model** of particle physics, a theory proposed and developed by many scientists over many years [@sm]. It's extrodinarily robust to experiments that have probed it thus far, and it is very much the norm for any budding physicist to pessimistically assume any search to uncover new physics will state "no excesses were found", with the result being "consistent with the Standard Model prediction". We go over an extremely brief overview of this model in the following paragraphs.
