...
$$
p(n|\lambda) = Poisson(\lambda)~.
$$ {#eq-poisson}

Along with the overall number of events we recorded, we may pick some variable of interest $x$ that we choose to measure. That variable will have a distribution we can predict from simulation. How do we describe it?
...
$$
p(\{x_i\}_{i=1}^n) = \mathrm{Poisson}(s+b)\prod_{i=1}^n\frac{sf_s(x_i) + bf_b(x_i)}{s+b}~.
$$ {#eq-histcomb}
...
$$
p(\{x_i\}_{i=1}^n | \mu) = \mathrm{Poisson}(\mu s+b)\prod_{i=1}^n\frac{\mu sf_s(x_i) + bf_b(x_i)}{\mu s+b}~.
$$
...
Say we histogram our simulated signal and background data with the same binning, giving us sets of counts $\mathbf{h}^{\mathrm{sig}} = \{h_1^{\mathrm{sig}}, h_2^{\mathrm{sig}}, \dots, h_s^{\mathrm{sig}}\}$ and $\mathbf{h}^{\mathrm{bkg}} = \{h_1^{\mathrm{bkg}}, h_2^{\mathrm{bkg}}, \dots, h_s^{\mathrm{bkg}}\}$. Recall from @sec-hists that we can use these to approximate a density by normalizing with respect to bin width and number of events, which we can write here as
