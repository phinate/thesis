...
$$
p(p(\{n_j\}_{j=1}^k | \mu) = \prod_{j=1}^k \mathrm{Poisson}(n_j | \mu h^{\mathrm{sig}}_j  + h^{\mathrm{bkg}}_j)~.
$$

This expression forms the core of the HistFactory approach. Now, we'll extend this expression in a similar way to when we addded $\mu$ in order to be able to model uncertainties.
