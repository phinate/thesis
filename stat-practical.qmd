...
$$
p(\{x_i\}_{i=1}^n) = Poisson(s+b)\prod_{i=1}^n\frac{sf_s(x_i) + bf_b(x_i)}{s+b}~.
$$

Notice that we don't have any free parameters right now -- the counts $s$ and $b$ will be fixed from our physics simulation. Now, here comes a little trick: if we wanted to infer the amount of signal present in our data, how would we do that? We can accomplish this by adding a pre-mu
