...
which is simply a product over Poisson distribution for each bin within a histogram, where we expect a contribution of $\mu h^{\mathrm{sig}}_j  + h^{\mathrm{bkg}}_j$ from each of signal and background respectively per bin $j$. This expression forms the core of the HistFactory approach. Now, we'll extend this expression in a similar way to when we addded $\mu$ to manipulate the signal scale, but this time, it's in order to be able to model uncertainties.
