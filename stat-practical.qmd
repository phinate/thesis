...
### The catalog of test statistics
...
$$
q_\mu= \begin{cases}-2 \ln \lambda(x, \mu) & \hat{\mu} \leq \mu \\ 0 & \hat{\mu}>\mu\end{cases}~.
$$
...
## Useful formulae from using the Asimov dataset

In @asymptotics, they proposed the notion of the **Asimov dataset**, which is defined for a given value of the signal strength $\mu$ as the dataset that would cause the fitted value $\hat{\mu}$ to equal $\mu$. As a simple example, assuming no background uncertainty in the model, this would just be the expected counts at that value of $\mu$, i.e. $\mu s+b$.

This may not appear that useful at first, but we can use this in conjunction with the results from @sec-asymptotics to produce some simple formulae that let us perform hypothesis tests without much computation at all. We start by remembering the result from Wald, which tells us the distribution of the test statistic $-2\ln \lambda(x,\mu) $ will be a non-central chi-squared distribution.
