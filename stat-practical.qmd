...
The first result we'll exploit to our advantage is that of Wilks @wilks, who showed that under the assumption of the null hypothesis $\mu=\mu_0$, we can relate $\lambda(x,\mu_0)$ with the maximum likelihood estimate $\hat{\mu}$ in the following way:

$$
-2\ln \lambda(x,\mu) = \left(\frac{\mu_0 - \hat{\mu}(x)}{\sigma_{\hat{\mu}}}\right)^2~,
$$

where $\sigma_{\hat{\mu}}$ is the sample standard deviation of $\hat{\mu}(x)$.
