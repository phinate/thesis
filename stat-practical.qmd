...
<!-- What happens when we break our assumption of $H_0$ being true, and want access to the  -->
...
We can also use our hypothesis test as a mechanism to set an **exclusion limit** on the signal strength $\mu$, which is conventionally done through an upper limit, as defined in @sec-conf-intervals. We could equally well look at other ordering choices, e.g. a central interval for $\mu$, though the upper limit may have more implications from a physics perspective as a yardstick for the maximum plausible amount of the signal that could exist in nature.
...
In @asymptotics, they proposed the notion of the **Asimov dataset**, which is defined for a given value of the signal strength $\mu'$ as the dataset that would cause the fitted value $\hat{\mu}$ to equal $\mu'$. As a simple example, assuming no background uncertainty in the model, this would just be the expected counts with $\mu=\mu'$, i.e. $\mu's+b$.

This may not appear that useful at first, but we can use this in conjunction with the results from @sec-asymptotics to produce some simple formulae that let us perform hypothesis tests without much computation at all. We start by remembering the result from Wald that says
