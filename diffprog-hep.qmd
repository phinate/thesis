...
Indeed, since all of these quantities can be calculated in a differentiable way, we can create a hybrid loss function using any of these components, such as $a_1 \mathrm{CL}_s + a_2\sigma_\mu + a_3\log{p_0} + ...$ etc. We can even view this as a regularization effect, where we have one clear loss target, then introduce other components with small linear coefficients to steer away from potentially undesirable pathologies. One example of a quantity that could serve this purpose is the "**Gaussianity**" of a likelihood, which my colleague/supervisor Lukas Heinrich coined as the mean-squared difference across a grid of points between the learned HistFactory likelihood and a Normal distribution defined using the covariance implied by the (inverse) Fisher information matrix (remember the Cramér–Rao bound from @sec-fisher).
