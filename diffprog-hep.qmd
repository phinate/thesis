...
We can see that training to minimze binary cross-entropy quickly overfits, since it's not too difficult to separate the nominal signal and background samples from each other in data space (teal vs orange points). However, note the difference in discovery $p$-value -- apparently we miss out on a $p$-value that's better by *six orders of magnitude* if we choose BCE over something like $p_0$ as an objective! It's also interesting to note that there's a very weak correlation between the models that are more inference-aware and their respective binary cross-entropies.
