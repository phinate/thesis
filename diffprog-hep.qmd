...
We know what happens to a KDE when we change the bandwidth, but how does that impact the bKDE? We can quantify this relative to the bin width by examining the shape of the bKDE relative to a "hard" histogram, which is shown in @fig-bkde-bandwidth. For low bandwidths, we recover something almost resembling a regular histogram. In fact, in the limit of zero bandwidth, we will *exactly* get a histogram! The reason is that zero bandwidth would turn each normal distribution into an infinite spike at each data point, which, when integrated over to get the counts, would have a contribution of 1 if the event lies in the bin, and 0 otherwise.
...
Now, the fisher information involves a matrix filled with second derivatives of the likelihood, and so requires data and parameters to be evaluated as a number. One thing we can do in the HistFactory setting is use Asimov data, which would mean we'll be able to know the best-fit parameter values already, and we can attempt to achieve the Cramér–Rao bound in @eq-cramer-rao by evaluating at $\mu=\mu'$, $x=x_A(\mu')$. As to the choice of $\mu'$, I'm not sure that it matters too much, as we're always going to be at the best fit parameter value for $\mu$ (the likelihood shape itself isn't affected by this, for instance), but I haven't studied this in detail.
...
### A loss landscape interlude for a two-bin model
...
- 4-layer neural network of size (128, 128, 128, 1)^[Initially, this was chosen to be less wide to maybe slow down the learning process a touch, though the extra layer mitigates that to some degree. The honest explanation is that this was just the config that happened to be there when things started working on the technical side.]
...
![Histograms from optimizing with respect to the $\mathrm{CL}_s$.](images/new-hist-models-CLs-5bin){#fig-hists-5bin-CLs}

![Histograms from optimizing with respect to a combination of discovery $p$-value and $\mathrm{CL}_s$.](images/new-hist-models-COMB-5bin){#fig-hists-5bin-comb}

![](images/new-hist-models-poi_uncert-5bin){#fig-hists-5bin-poi-uncert}


#### Neural network contours in data space {-}

![Histograms from optimizing with respect to the discovery $p$-value $p_0$.](images/new-grid-models-discovery-5bin){#fig-grid-5bin-discovery}

![Histograms from optimizing with respect to the $\mathrm{CL}_s$.](images/new-grid-models-CLs-5bin){#fig-grid-5bin-CLs}

![Histograms from optimizing with respect to a combination of discovery $p$-value and $\mathrm{CL}_s$.](images/new-grid-models-COMB-5bin){#fig-grid-5bin-comb}

![Histograms from optimizing with respect to the Fisher information estimate of $\sigma_{\hat{\mu}}$.](images/new-grid-models-poi_uncert-5bin){#fig-grid-5bin-poi-uncert}
...
![Histograms from optimizing with respect to the discovery $p$-value $p_0$.](images/new-hist-models-discovery-20bin){#fig-hists-20bin-discovery}

![Histograms from optimizing with respect to the $\mathrm{CL}_s$.](images/new-hist-models-CLs-20bin){#fig-hists-20bin-CLs}

![Histograms from optimizing with respect to a combination of discovery $p$-value and $\mathrm{CL}_s$.](images/new-hist-models-COMB-20bin){#fig-hists-20in-comb}

![Histograms from optimizing with respect to the Fisher information estimate of $\sigma_{\hat{\mu}}$.](images/new-hist-models-poi_uncert-bin){#fig-hists-20bin-poi-uncert}


#### Neural network contours in data space {-}

![Histograms from optimizing with respect to the discovery $p$-value $p_0$.](images/new-grid-models-discovery-20bin){#fig-grid-20bin-discovery}

![Histograms from optimizing with respect to the $\mathrm{CL}_s$.](images/new-grid-models-CLs-20bin){#fig-grid-20bin-CLs}

![Histograms from optimizing with respect to a combination of discovery $p$-value and $\mathrm{CL}_s$.](images/new-grid-models-COMB-20bin){#fig-grid-20bin-comb}

![Histograms from optimizing with respect to the Fisher information estimate of $\sigma_{\hat{\mu}}$.](images/new-grid-models-poi_uncert-20bin){#fig-grid-20bin-poi-uncert}
