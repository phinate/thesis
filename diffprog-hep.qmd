...
&= \sum_{\text{data }k} \frac{e^{y_{\text{bin } i}(x_k) / \tau}}{\sum_{j=0}^{\text{N}} e^{y_{\text{bin } j}(x_k) / \tau}}~,
\end{align}

where $\tau$ is a factor to tune how hard the slope of the softmax is, i.e. it has some analogy to the bandwidth about tuning the level of approximation. A drawback, however, is that the softmax used in tandem with a neural network does not come with an inherent notion of *ordering*; it's going to be somewhat arbitrary in which bins events occupy, and the resulting plots are a little hard to interpret compared to regular histograms. Also, even the "hard" softmax is difficult to map to some other "true" histogram in which the bins follow Poisson-distributed counts; I'm not 100% sure on whether the softmax counts
...
Many things to see here, but I'll just point out one of my favourites: for seed 3 in @fig-hists-20nobin-CLs and @fig-hists-20bin-discovery, we see the network struggling to untie the systematic variations from the signal bins. However, for  the same seed in @fig-hists-20bin-comb, where we're training on a combination of those metrics, the network seems to manage! No idea why this behaviour is there, and indeed it is just one seed. But I thought that was pretty cool.
