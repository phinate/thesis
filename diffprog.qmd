...
$$
\varphi^{(1)}_{i+1} = \varphi^{(1)}_{i} - \frac{\alpha}{\sqrt{\sum^{i}_{j=0}\left(g^{(1)}_{j}\right)^2 + \epsilon}} g^{(1)}_{i} ~,
$$

where $\epsilon$ is a very small number just to prevent dividing by zero. For $n$ parameters, we denote $g_{i}$ as the vector of all gradients $[g^{(1)}_{i}, \dots, g^{(n)}_{i}]$. Then, if we treat all the operations as *elementwise*, i.e. as one may expect with `numpy` arrays, we can write this again more compactly for all parameters $\varphi$ as

$$
\varphi_{i+1} = \varphi_{i} - \frac{\alpha}{\sqrt{\sum^{i}_{j=0}g^2_{j} + \epsilon}} g_{i} ~,
$$

<!-- Given input data $\mathbf{x}$ with multiple dimensions (features), it's very unlikely that these features will have equal importance for the task that you're trying to learn to perform well in. For instance, one could imagine recording the level of precipitation every day, but accidentally only doing this when the temperature hasn't varied much, then wanting to apply some neural network you train on this data as some weather prediction model. Clearly, the temperature is not going to be a reliable predictor of rainfall, so we don't want to waste much time exploring the loss landscape in the parameters that have learned something about the temperature, as they are unlikely to cause an impact on our performance. That's the kind of  -->
